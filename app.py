...
BOTANICAL_DB = {
    "Oak Tree": {
        ...
        "adults": {
            ...
            "destruction": "Deforestation eliminates a permanent ecosystem, destabilizes local water tables, and increases carbon footprint."
        }
    },
    "Snowdrop": {
...
# Age Selector for Adaptive Content - Professional UX Update
st.write("---")
selected_age = st.selectbox("🧒 Select Experience Mode", ["Explorer Mode 🧒", "Adults Mode 🧔"])
age_key = "kids" if "Explorer" in selected_age else "adults"
...
if uploaded_file is not None:
    ...
    # --- 💬 ADAPTIVE CONVERSATIONAL AI COMPONENT ---
    st.write("---")
    ...
    if prompt := st.chat_input("Ask something (e.g., What animals live in this tree?)"):
        ...
        with st.chat_message("assistant"):
            ...
            # ADAPTIVE RESPONSE PATHS GATED BY USER EXPERIENCE MODE
            if age_key == "kids":
                # Explorer Mode - Friendly, simple, storytelling tone
                if any(x in prompt_lc for x in ["animal", "insect", "bird", "squirrel", "nest"]):
                    ai_response = "🐿️ Beside fluffy squirrels, this big tree provides a safe home for cute owls, colorful woodpeckers, and hundreds of tiny bugs!" if detected_species == "Oak Tree" else "🐛 Tiny friendly bugs and early spring beetles find a warm shelter under my petals!"
                elif any(x in prompt_lc for x in ["water", "bulb", "soil", "care", "grow"]):
                    ai_response = "💧 Baby oak trees love a good drink of water when it's hot, but big grandpa oaks use giant roots to drink from deep underground!" if detected_species == "Oak Tree" else "🌱 We love soft, wet forest soil! Our tiny bulb keeps us warm all winter so we can wake up early."
                elif any(x in prompt_lc for x in ["bee", "honey", "nectar"]):
                    ai_response = "🐝 Yes! We are best friends with bees. We provide their very first yummy breakfast when spring begins!"
                else:
                    ai_response = f"🤖 What an awesome question! In the final app, the AI will tell you a cool story about this for {selected_age}!"
            else:
                # Adults Mode - Scientific, detailed, rigorous tone
                if any(x in prompt_lc for x in ["animal", "insect", "bird", "squirrel", "nest"]):
                    ai_response = "🦅 Quercus robur supports extensive biodiversity, hosting specialized avifauna like Picidae, Strigiformes, and thousands of distinct arthropod species." if detected_species == "Oak Tree" else "🪲 Galanthus ecosystems support early detritivores and soil microarthropods essential for spring nutrient cycling."
                elif any(x in prompt_lc for x in ["water", "bulb", "soil", "care", "grow"]):
                    ai_response = "💧 Saplings require regular irrigation during droughts. Mature specimens rely on an extensive taproot system extracting deep groundwater." if detected_species == "Oak Tree" else "🌱 Prefers organic, well-drained, moist soils. The subterranean bulb stores crucial carbohydrates required for premature vernal blooming."
                elif any(x in prompt_lc for x in ["bee", "honey", "nectar"]):
                    ai_response = "🐝 Correct. As a premier early-blooming flora, it provides critical nectar and pollen resources for Apis mellifera emerging from winter dormancy."
                else:
                    ai_response = f"🤖 Technical Query Documented. In the production layer, the integrated LLM API delivers real-time botanical analysis tailored for {selected_age}."
